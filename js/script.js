  ...
  for (let i = 1; i < data.length; i++) {
    ...
    for (let j = 0; j < data[i].length; j++) {
      if (j === 0) {
        ...
        html += `<a onclick="parseStyle('${data[i][1]}')" id = "albumArtist" href="#">${data[i][1]}</a><p></p>`
      } else if (j === 2) {
        html += `<a href="${data[i][5]}" target="_blank" id = "albumTitle">${data[i][2]}</a><p></p>`
      } else if (j === 3) {
        html += `Label: <a onclick="parseStyle('${data[i][3]}')" href="#">${data[i][3]}</a><p></p>`      
      } else if (j === 5) {
        html += `<a onclick="parseStyle('${data[i][7]}')" id = "musicGenre" href="#">${data[i][7]}</a>`      
      } else if (j === 6) {
        html += `, <a onclick="parseStyle('${data[i][8]}')" id = "musicStyle" href="#">${data[i][8]}</a><p></p>`
      } else if (j === 7) {
        html += `<a onclick="parseStyle('${data[i][9]}')" id = "musicStyle" href="#">${data[i][9]}</a>, *${data[i][15]}*<p></p>`
      } else if (j === 8) {
